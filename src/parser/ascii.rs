...
pub fn parse_ascii_response(buf: &[u8]) -> Result<Option<(usize, Response)>, ErrorKind> {
    ...
    match result {
        ...
        Err(nom::Err::Error(e)) | Err(nom::Err::Failure(e)) => Err(ErrorKind::Protocol(Some(format!("{:?}", e)))),
    }
}
    ...
    terminated(
        map_res(
            ...
            |(_, key, _, value)| {
                ...
                Ok::<_, Utf8Error>(StatsResponse::Entry(keystr.to_string(), valuestr.to_string()))
            },
        ),
        crlf,
    )(buf)
...
pub fn parse_ascii_metadump_response(buf: &[u8]) -> Result<Option<(usize, MetadumpResponse)>, ErrorKind> {
    let bufn = buf.len();
    let result = alt((
...
mod tests {
    use super::{
        parse_ascii_metadump_response, parse_ascii_response, parse_ascii_stats_response, ErrorKind, KeyMetadata,
        MetadumpResponse, Response, StatsResponse, Status, Value,
    };
